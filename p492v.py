...
def DetectLanes_FitPoly(binary_warped, debug):
...
def SearchSubsequentFrames (binary_warped, left_fit, right_fit, out_img, debug):
    nonzero = binary_warped.nonzero()
    nonzeroy = np.array(nonzero[0])
    nonzerox = np.array(nonzero[1])
    margin = 100
...
def CalcRadOfCurvature(ploty, left_fit, right_fit): #Left fit and right fit are actual plottable x and y vals, not the 'other' polyfit returned values
...
while True:
    ...
    if (video == 1 and framenum == 1):
        ...
        left_fit, right_fit, out_img = DetectLanes_FitPoly(binary_warped, debug)
    
    elif (video == 1 and framenum > 1):
        #print('Calling subsequent lane detection routine ', framenum)
        #Next check if prev. procedure did not produce lane lines, then need to call Sliding windows routine
        ploty, left_fitx, right_fitx = SearchSubsequentFrames(binary_warped, left_fit, right_fit, out_img, debug)
        #Have to watch for RankWarnings from the above polyfit which indicates a bad fit; not implem yet
        #print (left_fitx, '\n', right_fitx)
        l_radius, r_radius, off_center = CalcRadOfCurvature(ploty, left_fitx, right_fitx)
        #if ret == 0:
        ...
        cv2.putText(res, name, (1100, 15), font, 0.5, (255,0,0), 0, cv2.LINE_AA)
